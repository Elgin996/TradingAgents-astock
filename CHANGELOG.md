...
- 补上 `pyproject.toml` 与 CHANGELOG 的版本号：**v0.3.0 发布时两处都漏改**，
  pyproject 一直停在 0.2.21。
...
（补记：本版发布时漏更 CHANGELOG 与 `pyproject.toml` 版本号，于 0.3.1 补上。）

**破坏性变更**：出于合规考虑删除可执行价位能力（`entry_price` / `stop_loss` /
`position_sizing` / `price_target` 四个字段），Trader 与 Portfolio Manager 只给
方向与理由。另修正提示词中 8 处 A 股交易规则错误（#85，感谢
[@cheneyliu0304](https://github.com/cheneyliu0304)）、修复 920 号段被误判为沪市、
新增 Docker Web UI 服务（#82）。完整说明见
[Release v0.3.0](https://github.com/simonlin1212/TradingAgents-astock/releases/tag/v0.3.0)。
