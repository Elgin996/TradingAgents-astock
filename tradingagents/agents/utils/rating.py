...
def parse_rating(text: str, default: str = "Hold") -> str:
    ...
    # A Chinese rating label followed by an English-looking value that is not
    # a standalone tier (for example ``Buy-side`` or ``Buy2024``) must not
    # fall through to the prose scanner and get misclassified as Buy.
    invalid_cn_english = re.search(_CN_LABEL_PREFIX + r"([A-Za-z]+)", text)
    if invalid_cn_english:
        return default
