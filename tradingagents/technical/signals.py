...
def assess_signals(
    ...
    *,
    ...
    observe_only_grades: Iterable[str] = ("C",),
    block_grades: Iterable[str] = ("D", "F"),
) -> TechnicalAssessment:
    signals = list(signals)
    usable = [signal for signal in signals if signal.direction != "unavailable" and signal.confidence > 0]
    if not usable or quality_grade in set(block_grades):
    ...
    category_weights = dict(category_weights or {"trend": 0.30, "momentum": 0.25, "volume": 0.15, "volatility": 0.15, "level": 0.15})
    ...
    if quality_grade in set(observe_only_grades):
...
class SignalEngine:
    ...
    def run(self, bars_or_frame, *, quality_grade: str = "A", snapshot_id: str = "") -> SignalEvaluation:
        ...
        assessment = assess_signals(
            ...
            category_weights=self.rules.get("weights"),
            observe_only_grades=self.rules.get("observe_only_grades", ("C",)),
            block_grades=self.rules.get("block_grades", ("D", "F")),
        )
