"""Data quality gate integration test: live a_stock.py endpoints.

Requires network access to Eastmoney/Sina/etc. — excluded from default CI via
``-m "not integration"``.
"""
...
def test_a_stock_data_quality_endpoints():
    results = _run_endpoint_suite()
    fails = [r for r in results if r[1] == "FAIL"]
    assert not fails, f"{len(fails)} endpoint(s) failed: {fails}"
